...
############################### Figure sup 1 , Association of PGS height with presentation in individuals from UK biobank
...
Box_plot <- function (PRS,Disease,subset,age=0,test="t.test") {
    ...
    #add cmrb statues without AD
    #data_cmrb <- DATA[DATA[[subset_carga]] == 0 & DATA[[Disease]] ==1  ]
    #data <- rbind(data_cmrb, data_AD) 
    ...
    scale_fill_manual(values=c(mypal3,mypal,mypal2) )
...
#Make plots
p1 <- Box_plot("Height","T2D","AD",test ="anova",age=65) %>% label_plot(.,"A",25)
p2 <- Box_plot("Height","MDD","AD",test ="anova",age=65) %>% label_plot(.,"B",25)
p3 <- Box_plot("Height","MH","AD",test ="anova",age=65)  %>% label_plot(.,"C",25)
p4 <- Box_plot("Height","EPI","AD",test = "anova",age=65)  %>% label_plot(.,"D",25)
...
ggsave(file="All_figures/FigureSup1.pdf",grid_vertical,width = 11,height = 11)
